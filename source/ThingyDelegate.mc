...
class ThingyDelegate extends BluetoothLowEnergy.BleDelegate {
    ...
    public function onScanResults(scanResults as Iterator) as Void {
        for (var result = scanResults.next(); result != null; result = scanResults.next()) {
            if (result instanceof ScanResult) {
                // Check if the scan result contains your custom service UUID
                if (contains(result.getServiceUuids(), _profileManager.NANO_SERVICE_UUID)) {
                    // Broadcast the scan result or handle accordingly
                    broadcastScanResult(result);
                }
            }
        }
